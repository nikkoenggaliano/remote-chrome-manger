...
function buildLaunchState(instance, options = {}) {
  ...
  const useXvfbBackend = Boolean(instance?.use_xvfb);
  ...
  if (useXvfbBackend && xvfbSupported && !options.forceHeadless) {
    return {
  ...
  return {
    ...
    launch_reason: reason || (
      useXvfbBackend
        ? (xvfbSupported ? 'xvfb_fallback_headless' : 'platform_without_xvfb')
        : 'native_chrome_headless'
    ),
  };
}
...
async function spawnInstance(id) {
  ...
  if (isTruthy(process.env.CHROME_MANAGER_FORCE_HEADLESS) || (process.platform === 'darwin' && isTruthy(process.env.CHROME_MANAGER_DARWIN_HEADLESS))) {
    appendLog(logFile, 'INFO', 'Legacy global headless env detected; local instances are already launched headless and now choose only the backend (Xvfb or Chrome Headless).');
  }
