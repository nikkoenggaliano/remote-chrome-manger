...
async function spawnInstance(id) {
  ...
  appendLog(
    logFile,
    'INFO',
    `Launch mode resolved: mode=${launchState.launch_mode}, xvfb=${launchState.xvfb_enabled ? 'on' : 'off'}, headless=${launchState.headless_enabled ? 'on' : 'off'}, reason=${launchState.launch_reason}`
  );
  if (launchState.launch_mode === 'gui') {
  ...
  const useHeadless = launchState.headless_enabled;
