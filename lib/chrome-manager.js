...
async function spawnInstance(id) {
  ...
  // Keep non-headless by default for WebGL on macOS.
  // You can force headless on any platform with CHROME_MANAGER_FORCE_HEADLESS=1.
  const forceHeadless = process.env.CHROME_MANAGER_FORCE_HEADLESS === '1';
  const useDarwinHeadless = process.platform === 'darwin' && process.env.CHROME_MANAGER_DARWIN_HEADLESS === '1';
  const useHeadless = forceHeadless || useDarwinHeadless;
  ...
  if (enableWebGL) {
    ...
    if (useHeadless) {
      ...
      chromeArgs.push('--use-angle=swiftshader-webgl');
  ...
  if (useHeadless) {
    ...
    appendLog(logFile, 'INFO', `Headless mode enabled (force=${forceHeadless}, darwinFlag=${useDarwinHeadless}, instance.use_xvfb=${use_xvfb})`);
  }
