...
function buildLaunchState(instance, options = {}) {
  ...
  const headlessStackRequested = Boolean(instance?.use_xvfb);
  ...
  if (!headlessStackRequested) {
    return {
      launch_mode: 'gui',
      launch_mode_label: 'Xvfb = Headless OFF',
      launch_backend: 'gui',
      ...
      headless_stack_requested: false,
      launch_state_source: source,
      launch_reason: reason || 'instance_toggle_off',
    };
  }

  if (xvfbSupported && !options.forceHeadless) {
    return {
      launch_mode: 'headless',
      launch_mode_label: 'Xvfb = Headless ON',
      launch_backend: 'xvfb',
      launch_backend_label: 'Xvfb',
      headless_stack_enabled: true,
      xvfb_process_enabled: true,
      chrome_headless_enabled: false,
      xvfb_enabled: true,
      headless_enabled: true,
      headless_stack_requested: true,
      launch_state_source: source,
      launch_reason: reason || 'instance_toggle_on_xvfb',
    };
  }
  ...
  return {
    ...
    headless_stack_requested: true,
    launch_state_source: source,
    launch_reason: reason || (xvfbSupported ? 'xvfb_fallback_headless' : 'platform_without_xvfb'),
  };
}
...
async function spawnInstance(id) {
  ...
  const legacyForceHeadless = isTruthy(process.env.CHROME_MANAGER_FORCE_HEADLESS);
  const legacyDarwinHeadless = process.platform === 'darwin' && isTruthy(process.env.CHROME_MANAGER_DARWIN_HEADLESS);
  const legacyHeadlessRequested = legacyForceHeadless || legacyDarwinHeadless;
  ...
  if (!use_xvfb && legacyHeadlessRequested) {
    appendLog(
      logFile,
      'INFO',
      `Per-instance headless toggle is OFF. Ignoring legacy global headless env (force=${legacyForceHeadless}, darwinFlag=${legacyDarwinHeadless}).`
    );
  }
  ...
  if (launchState.launch_backend === 'gui' && process.platform === 'linux' && !hasGuiDisplay) {
    const errorMessage = 'Cannot start with "Use Xvfb / Headless" OFF on this Linux host because DISPLAY/WAYLAND_DISPLAY is empty. Turn the toggle ON or run the server inside a desktop session that provides a display.';
  ...
  if (launchState.xvfb_process_enabled) {
    try {
      ...
      display = null;
      delete env.DISPLAY;
      launchState = buildLaunchState(instance, {
        source: 'runtime',
        forceHeadless: true,
        reason: 'xvfb_unavailable_fallback_headless',
      });
      appendLog(logFile, 'WARN', `Xvfb unavailable (${xvfbBin}): ${err.message}. Falling back to Chrome headless.`);
      console.warn(`Xvfb (${xvfbBin}) not found or failed to start, falling back to Chrome headless`);
    }
  }
  ...
  if (launchState.launch_backend === 'gui') {
    appendLog(logFile, 'INFO', 'GUI mode selected because the per-instance headless toggle is OFF.');
  }
