...
async function spawnInstance(id) {
  ...
  // Enable this only as fallback if machine can't keep GUI Chrome alive.
  ...
  if (useDarwinHeadless) {
    ...
    appendLog(logFile, 'INFO', `Darwin fallback mode enabled: launching with --headless=new (instance.use_xvfb=${use_xvfb})`);
  }
  ...
  const darwinUseOpen = process.platform === 'darwin' && !useDarwinHeadless && process.env.CHROME_MANAGER_DARWIN_USE_OPEN !== '0';
