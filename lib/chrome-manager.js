...
async function spawnInstance(id) {
  ...
  const useDarwinHeadless = process.platform === 'darwin' && !use_xvfb && !disableDarwinHeadless;
  if (useDarwinHeadless) {
    chromeArgs.push('--headless=new');
    chromeArgs.push('--hide-scrollbars');
    chromeArgs.push('--mute-audio');
    appendLog(logFile, 'INFO', 'Darwin safe mode enabled: launching with --headless=new (set CHROME_MANAGER_DISABLE_DARWIN_HEADLESS=1 to disable)');
  }
