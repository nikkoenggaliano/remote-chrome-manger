...
// Sum resident memory (RSS, in bytes) for a process subtree rooted at the given
// PIDs. A single `ps` snapshot keeps this portable across macOS and Linux, where
// RSS is reported in KB. Returns null if none of the roots are alive.
function getProcessTreeRssBytes(rootPids) {
  const roots = (Array.isArray(rootPids) ? rootPids : [rootPids]).filter(Number.isFinite);
  if (!roots.length) return null;

  let rows;
  try {
    rows = execSync('ps -A -o pid=,ppid=,rss=', { stdio: ['ignore', 'pipe', 'ignore'] })
      .toString()
      .trim()
      .split('\n');
  } catch {
  ...
  while (queue.length) {
    ...
    seen.add(pid);
    if (rssKbByPid.has(pid)) {
      totalKb += rssKbByPid.get(pid);
      counted = true;
    }
...
// Resident memory (bytes) used by a running local instance and its child
// processes (renderers, GPU, helpers). Remote instances run on another host, so
// their memory can't be measured locally and this returns null.
function getInstanceMemoryBytes(instance) {
  if (!instance || instance.status !== 'running') return null;
  if (instance.type && instance.type !== 'local') return null;
  const pids = getListeningPids(instance.port);
  if (!pids.length) return null;
  return getProcessTreeRssBytes(pids);
}
...
async function syncStatuses() {
    ...
    for (const inst of allInstances) {
        ...
        if (!isAlive) {
                 ...
                 db.prepare("UPDATE instances SET status = 'stopped', display = NULL, started_at = NULL WHERE id = ?").run(inst.id);
             }
        }
    }
}
