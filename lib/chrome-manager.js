...
async function spawnInstance(id) {
  ...
  if (use_xvfb) {
    try {
      execSync('command -v Xvfb');
      display = findFreeDisplay();
      env.DISPLAY = display;
      xvfbProc = spawn('Xvfb', [display, '-screen', '0', '1920x1080x24', '-ac', '+extension', 'GLX', '+render', '-noreset']);
      // Wait for Xvfb to be ready
      await new Promise(resolve => setTimeout(resolve, 1000));
    } catch (err) {
      console.warn('Xvfb not found or failed to start, falling back to default display or headless');
    }
  }
