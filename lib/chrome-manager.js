...
async function spawnInstance(id) {
  ...
  // macOS servers/ssh sessions often crash non-headless Chrome with SIGTRAP
  // (WindowServer / NSNotificationCenter not available). Use headless by default.
  const disableDarwinHeadless = process.env.CHROME_MANAGER_DISABLE_DARWIN_HEADLESS === '1';
  // On macOS we skip Xvfb anyway, so enforce headless regardless of use_xvfb flag.
  const useDarwinHeadless = process.platform === 'darwin' && !disableDarwinHeadless;
  if (useDarwinHeadless) {
    chromeArgs.push('--headless=new');
    chromeArgs.push('--hide-scrollbars');
    chromeArgs.push('--mute-audio');
    appendLog(logFile, 'INFO', `Darwin safe mode enabled: launching with --headless=new (instance.use_xvfb=${use_xvfb}) (set CHROME_MANAGER_DISABLE_DARWIN_HEADLESS=1 to disable)`);
  }
